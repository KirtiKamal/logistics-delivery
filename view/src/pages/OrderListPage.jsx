...
const OrderListPage = () => {
  ...
  useEffect(() => {
    console.log("Fetching orders, customers, and vehicles with token:", token);
  ...
  const generateUniqueOrderNumber = () => {
    ...
    const orderNumber = `UP80-${currentDate.getFullYear()}${currentDate.getMonth() + 1}${currentDate.getDate()}-${randomNumber}`;
    return orderNumber;
  };
  ...
  return (
    <div className="order-list-container">
      ...
      {error && <div className="error-message-order">{error}</div>}
      <div className="info-icon-container">
        <FcInfo className="info-icon" />
        <div className="info-content">
          Please visit Items, Customers, and Vehicle page to see availability.
        </div>
      </div>
